...
| R-04 | Cancellation doesn't cancel / leaves strays | **CLOSED upstream** (#373/#375); kill semantics re-verified end-to-end by S-01 (TERM→`terminated`, in-flight `sleep` died) | — | — | `streamExec({newProcessGroup})` + `killExecProcessGroup` (TERM→poll→KILL, race-free pgid); smoke-test Phase 6 | **Confirmed residual (S-01): 1 unreaped zombie per killed run** vs `PidsLimit: 1024` (Q-08, upstream #381); daemon children that leave the process group | — |
