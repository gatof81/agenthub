...
| R-04 | Cancellation doesn't cancel / leaves strays | **CLOSED upstream** (#373/#375); kill semantics re-verified by S-01 (TERM→`terminated`, exit 15, no post-TERM flush) | — | — | `streamExec({newProcessGroup})` + `killExecProcessGroup` (TERM→poll→KILL, race-free pgid); smoke-test Phase 6 | **Two confirmed residuals (S-01):** (a) 1 unreaped zombie per killed run vs `PidsLimit: 1024` (Q-08, upstream #381); (b) **Bash-tool children escape the group by default** — killing the `claude` group leaves its in-flight shell command running; runner post-cancel policy required (doc 08) | — |
