...
| N5b-2 | **Worktree isolation + task view**: run each task in its own git worktree (ADR-010 B) via `DelegatedWorkspaceAccess` over the exec seam, so concurrent tasks don't collide in the project session; frontend task view (steps, reports, live progress) | `orchestrator/` (worktree-over-exec), `api` + frontend task view | inherited | 05, 06, 11, ADR-010 |
