...
| `POST /api/specialists/:id/session` | bind or create a specialist's personal session (N3b-1, ADR-008) | body: exactly one of `{sessionId}` (bind existing owner session) or `{sessionTemplateId}` (create on-behalf, #420); `422` on both-or-neither, `404` unknown specialist, `409 session_gone` if the bound session is absent upstream. Returns `201 {session}`. Direct conversation with a specialist is N3b-2 |
