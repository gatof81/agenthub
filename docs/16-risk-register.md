...
| R-03 | Headless run freezes forever on first tool permission prompt when no permission flags are passed | **OPEN** → S-01 confirms | H (if unhandled) | M | Runner always passes an explicit permission policy (Q-02, sign-off pending); per-run wall-clock timeout as backstop | — | 1 |
...
| R-05 | Prompt injection → tool misuse in unattended headless runs (repo/web content steers the agent; nobody is watching) | OPEN | M | H | Q-02 restrictive default posture; secrets kept out of prompts; audit trail (events + activity) from day 1; autonomy levels + approvals in later phases | Container has open egress; workspace may hold PATs/.env → sandbox is **not** an exfil boundary. Restate in doc 10 | 1→3 |
...
| --- | --- | --- |
| old R-08 / R-14 | D1 cost/latency blowup; D1 non-transactionality → inconsistent run state | Premised on sharing the substrate's D1. Superseded by separate-repos + Hub-owned persistence (ADR-002). If ADR-002 chooses a Hub-owned D1, both revive and S-03 runs before the ADR closes |
