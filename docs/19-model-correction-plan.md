...
| --- | --- | --- | --- | --- |
| N1 | **Session discovery & ownership**: list the owner's sessions with state/ownership, open them ([#419](https://github.com/gatof81/shared-terminal/issues/419)); substrate re-pin | `domain/ports.ts` (+list/get), `substrate/real.ts` + double + conformance, `api/app.ts` (GET /api/sessions), frontend sessions view | none (#419 improves UX) | 02 re-verified at new pin; contract doc |
| N2 | **Project binding**: bind an existing owner session (create nothing); create-new in the owner's account; migration 004 | `domain/types.ts`, `orchestrator.ts` (bind path in `createProject`), `store/migrations/004`, `api` (POST /api/projects), frontend session selector | [#416](https://github.com/gatof81/shared-terminal/issues/416) to execute; [#420](https://github.com/gatof81/shared-terminal/issues/420) for create-new | 04 (FR-30/40/45), 05 (UC-01), 08, 09 |
| N3 | **Specialists**: Claudio/Claudia registered, bound to visible sessions, direct conversation with each; migrations 005–006 | `config/agents.ts` → specialists, `store/migrations/005-006`, `api` (GET /api/specialists…), frontend Specialists nav | inherited | 06, 08, 11 |
