...
| `POST /api/projects` | create + provision (202) | `{name, defaultAgentId, sessionTemplateId?, repo?, instructions?}` — `repo` = `{url, ref?, target?}` + an `auth` sibling the route splits off and hands to the seam, never to the store — it is not a field of the stored repo (FR-45/47, SEC-11). The workspace comes from the **project**, not the agent (ADR-006). Credentials are provisioned here, never authenticated inside a turn (FR-46) |
