/**
 * RunLoop (ADR-013): the dispatch → execute → terminal engine, and the sole
 * owner of `finalize` — the 09 §3 terminal choke point every outcome flows
 * through. Owns the run-state maps (`inFlight`, `cancelRequested`, `timedOut`,
 * `budgetTripped`, `pendingKills` — the B3-01 kill-outcome race lives here),
 * the FR-21 post-cancel sweep, the FR-25 wall-clock backstop, cancellation
 * (UC-04), the run half of boot reconciliation and the queue rebuild (UC-06).
 *
 * It executes turns; it never decides them. WHERE a run executes (and the
 * ADR-014 envelope signals) is `resolveTarget`, injected from the facade —
 * the wiring of SessionResolver + TaskCoordinator; `sessionMeta` (lookup for
 * kill/reconcile/error context) and `onRunTerminal` (the task-step awaiter
 * wake) are injected the same way, so the collaborators stay acyclic.
 */
...
import { assembleAssistantText, deriveRunSummary } from '../domain/projections.js';
...
export interface RunLoopDeps {
  ...
  /** post-cancel sweep plumbing only (FR-21) — turns go through the adapter */
  execPort: SubstrateExecPort;
  /**
   * Resolve where a run executes and route the envelope signals — the
   * facade's wiring of SessionResolver + TaskCoordinator (ADR-013). Null =
   * the run was finalized without a turn (fail/kickoff/steer).
   */
  resolveTarget: (
    run: Run,
  ...
  /** Session lookup (never started) for kill/reconcile/error context — SessionResolver's. */
  sessionMeta: (c: Conversation) => { sessionId: string | null; lastKnownState: string | null };
  /** Wake a task-step awaiter on every sealed run — TaskCoordinator.wakeRunTerminal. */
...
export class RunLoop {
  ...
  private readonly resolveTarget: RunLoopDeps['resolveTarget'];
  private readonly sessionMeta: RunLoopDeps['sessionMeta'];
  private readonly onRunTerminal: RunLoopDeps['onRunTerminal'];
  ...
  constructor(deps: RunLoopDeps) {
    ...
    this.resolveTarget = deps.resolveTarget;
    this.sessionMeta = deps.sessionMeta;
  ...
  /** The in-flight run promises, for the facade's `idle()` (deterministic tests, clean shutdown). */
    ...
    if (run.state === 'starting' || run.state === 'streaming') {
      ...
      if (run.execId) {
        ...
        const pending = this.adapter.kill(
          this.sessionMeta(conversation).sessionId!,
          run.execId,
          KILL_GRACE_MS,
        );
  ...
  // — UC-06: boot reconciliation, the run half (two transactions per run) —
  ...
  async reconcileRuns(): Promise<void> {
    ...
    for (const run of this.store.listRunsByState(['interrupted'])) {
      const conversation = this.store.getConversation(run.conversationId)!;
      const message = this.store.getMessage(run.messageId)!;
      const sessionId = this.sessionMeta(conversation).sessionId;
      const probe =
  ...
  /** Queue rebuild (UC-06): re-arm every WORKSPACE that still has queued runs (projects and specialist workspaces alike, N3b-2). */
  rebuildQueues(): void {
  ...
  private async executeRun(run: Run): Promise<void> {
    ...
    try {
      ...
      const lastKnownState = this.sessionMeta(conversation).lastKnownState ?? 'unknown';
  ...
  // — shared terminal-transition helper (09 §3: one transaction). Public:
  // the facade's fail-signal seal and the TaskCoordinator's envelope seal go
  // through this same choke point (ADR-013) —
