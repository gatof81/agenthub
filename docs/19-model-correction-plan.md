...
| N3b-2 ✅ | **Direct conversation with a specialist** (done, PR #70): conversation `mode` + nullable `projectId` (migration 006), a run resolves its session from the specialist's binding (starting it if offline — needs shared-terminal#429), runs serialize per workspace (I-2), frontend chat | `domain/types.ts` (+`workspaceKeyFor`), `store/migrations/006` + `migrate` FK toggle, `orchestrator` (`createSpecialistConversation`, `resolveRunSession`, per-workspace dispatch), `api`, frontend | inherited (start: [#429](https://github.com/gatof81/shared-terminal/issues/429)) | 06, 08 |
| N4a ✅ (PR #86) | **Routing decision core** (offline): a deterministic router that proposes the conversation's own specialist — its pin/default, **no model call** — plus the pure execution-target selector and the `ExecutionTargetDecision` recorded on the run and surfaced in the inspector. Automatic mode is wired end-to-end and fully offline-testable, but the default conversation mode stays `direct` (the flip waits for real routing in N4b) | `orchestrator/` (`router.ts` + `selector.ts`), `domain/types.ts` (`RouteProposal`/`ExecutionTargetDecision`), `domain/ports.ts` (`RouterPort`), `store/migrations/007` + `store/{sqlite,memory}` (run `target*` columns) + contract suites, frontend inspector | none (upstream all shipped) | 07 §2 (selector as the 4th deterministic function), 08, 09 |
| N4b ✅ (PR #93) | **Model router** (ADR-012): a message-aware `RouterPort` implementation (real mode) that reads the message and picks a specialist by capability — one cheap Messages-API call to Haiku 4.5 with structured output, **reusing `CLAUDE_CODE_OAUTH_TOKEN`** (a narrow control-plane carve-out from ADR-001, not the exec seam). The N4a deterministic router is both the offline fake **and** the fallback on model error/timeout/unknown-specialist. Flips the default conversation mode to `automatic` (new project conversations; existing keep their mode) | `orchestrator/modelRouter.ts` behind `RouterPort`, `main.ts` (real vs fake wiring), `@anthropic-ai/sdk`, frontend mode badge | inherited | ADR-012, 05 (new UC — automatic routing), 08 |
| N5a ✅ (PR #98) | **Task data core** (offline): the Task entity + `TaskState` machine (ADR-009, `planning → implementing → qa_pending → qa_running → (changes_requested_by_qa → implementing)* → awaiting_human_approval → approved \| changes_requested_by_user → implementing \| rejected \| failed`), TaskStep, and the `ImplementationReport`/`QaReport` work products (18 §4 envelope); store CRUD + contract suite; migration 008 (`tasks`, `task_steps`, `work_products`, `runs += task_step_id`). No supervisor yet — pure data + machine, fully offline-testable | `domain/taskStateMachine.ts` + task types, `store/migrations/008` + `store/{sqlite,memory}` + contract suite | inherited | 06, 09 |
| N5b-1 ✅ (PR #101) | **Developer → QA supervisor core** (strategy A — the project's primary session): a routed `task` message spawns a supervised task. The supervisor drives one hardcoded flow — developer (implement → `ImplementationReport`) → QA (review → `QaReport`) with the bounded `changes_required` loop → `awaiting_human_approval` (or `failed`) — enforcing `TaskState` (never a model deciding authority; a QA `verdict` only routes the loop). The turn machinery is behind a `StepRunner` seam so the loop is unit-tested offline; the real `StepRunner` wraps the run loop (each step is its own run, linked via `runs.task_step_id`), and a task-envelope kickoff run spawns the task without executing a turn. **Work-product decision (was open at N5b): a separate extraction** — a `ReportExtractorPort` turns each step's output into the typed work product (mechanical/offline fake; a Haiku forced-tool-call in `real`, reusing the OAuth token like the router, ADR-012). QA extraction failure is fail-safe to `changes_required`, never auto-pass. Envelope is opt-in via `HUB_QA_SPECIALIST` (prod-safe: unset → no tasks) | `orchestrator/{supervisor,reportExtractor}.ts`, `orchestrator/orchestrator.ts` (envelope, `StepRunner`, run-completion signal), `store` (`sendMessage += taskStepId`), `main.ts` | inherited | 05, 06, 13, 18 §6, ADR-009/012 |
| N5b-2a ✅ (PR #104) | **Worktree isolation** (ADR-010 B, backend): each task's steps run inside a git worktree/branch owned by the project session — plain `git worktree` over the exec seam (no new substrate capability), created before the dev step, committed after it (so QA reviews a committed ref and work survives specialist failure), removed on a terminal task state (the branch/commits survive for N6's PR). The seam-native `workingDir` exec field runs a step's turn in its worktree; the deterministic selection (worktree-write / test-execution by step kind, ADR-008) is recorded as a `DelegatedWorkspaceAccess` snapshot on the step (migration 009). Behind a `WorkspaceManagerPort` (real git / offline fake), so the supervisor's create → commit → cleanup orchestration is offline-tested; graceful strategy-A fallback for a repo-less project | `orchestrator/workspaceManager.ts` + `domain/ports.ts` (`WorkspaceManagerPort`), `ExecRequest`/`TurnRequest` `workingDir`, `substrate/real.ts`, `store/migrations/009` + contract suite, `main.ts` | inherited | 05, 06, 13, ADR-010 |
| N5b-2b ✅ (PR #105) | **Task view** (frontend): render a task — its steps, work products (`ImplementationReport`/`QaReport`), the worktree ref + `DelegatedWorkspaceAccess` audit, and live progress; leaks visible in the inspector (ADR-010) | `api` (task read endpoints) + frontend task view | inherited | 05, 11, ADR-010 |
| N6a ✅ (PR #106) | **Human approval machine**: from `awaiting_human_approval` the owner approves (→ `approved`, terminal), rejects (→ `rejected`, terminal), or requests changes (→ `changes_requested_by_user` → the supervisor RE-ENTERS the dev → QA loop with the note as feedback, reusing the existing worktree, back to `awaiting_human_approval`). Approve/reject clean up the worktree (branch/commits survive); the objective + specialists are recovered from the task's source message + steps. API (`approve`/`reject`/`request-changes`) + the verdict actions in the `TaskView`. Offline-tested (supervisor resume; approval API incl. the re-run end-to-end on the fake substrate) | `orchestrator` (approve/reject/requestChanges + supervisor resume), `api`, frontend TaskView actions | inherited | 05, 08, 11 |
| N6b ✅ (PR #108) | **Approve → open the PR**: on approve, the project session pushes the task branch and runs `gh pr create` with its own repo credential (ADR-010: PRs from the project session, never a specialist); the PR URL is recorded and shown in the TaskView | `orchestrator`/`WorkspaceManagerPort` (git push + gh), `api`, frontend | inherited | ADR-010, 05, 11 |
