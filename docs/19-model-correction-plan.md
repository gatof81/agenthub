...
| N4b | **Model router**: a message-aware `RouterPort` implementation (real mode) that reads the message and picks a specialist by capability; the N4a deterministic router becomes both the offline fake **and** the fallback when the model errors or times out. Flips the default conversation mode to `automatic` | `orchestrator/` (model router behind `RouterPort`), `config/runtime.ts` (real vs fake wiring), frontend automatic-mode affordance | inherited | 05 (new UC — automatic routing), 08 |
