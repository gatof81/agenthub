...
| N5b | **Developer → QA supervisor**: create a Task from a routed `task` message; the `orchestrator/` supervisor drives Claudio (implement → `ImplementationReport`) → Claudia (QA → `QaReport`) with the `changes_required` loop → `awaiting_human_approval`; worktree strategy (ADR-010 B) + `DelegatedWorkspaceAccess`; frontend task view. How work products are produced (specialist emits structured vs. a separate extraction) is decided at N5b | `orchestrator/` (supervisor), worktree-over-exec, `api` + frontend task view | inherited | 05, 06, 13 |
