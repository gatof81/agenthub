...
| SEC-11 | M | The Hub holds a credential that can **write to the owner's source repositories** — a capability nothing else it holds has (the substrate JWT reaches sessions, the Anthropic token bills, the R2 token reaches snapshots; none touch source). Blast radius is bounded by construction: one fine-grained PAT per repository, so a leak reaches one repo, and revocation is per project. The Hub stores no plaintext — repo auth is encrypted by the seam via `encryptAuthCredentials` (`sessionConfig.ts:1138`, verified at shared-terminal `main @ c35b6da`; upstream's own note: *"the whole point of `auth_json` / `encryptAuthCredentials` is that secrets never land in plaintext anywhere on disk"*) | owner decision (2026-07-16), ADR-006, 10 §2 |
