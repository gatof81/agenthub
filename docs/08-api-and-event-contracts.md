...
| `POST /api/projects/:id/conversations` | create conversation in the project (instant — no provisioning, ADR-005) | body `{title?, agentId?, mode?}` (agent defaults from project). `mode` ∈ `automatic` (default since N4b — routes each turn, ADR-008/012) \| `direct` (pins `agentId`, no model call); any other value → `422 validation`. Returns `201 {conversation}` |
...
| `GET /api/conversations/:id` | detail + messages (paged) + `hasMore` | `?before=<messageId>&limit=` returns the newest `limit` (older with `before`); `hasMore` flags that earlier messages exist so the client can offer "load earlier" (E). An assistant message whose turn used tools also carries `segments` — the ordered `text`/tool bubbles derived from the run's events (A) — so a reloaded turn renders as the same sequence a live one does; `content` stays the single-blob fallback |
...
| `GET /api/conversations/:id/runs` | the conversation's **run history**, newest first (the activity panel: running on top, executed below) | `?limit=` (default 50, max 200) + `hasMore`; entries are `{run, summary}` where `run` is **lean** (lifecycle, outcome, `targetDecision`, `taskStepId`, timestamps — never the caps/policy/instructions snapshots or seam handles) and `summary` is the light outcome (`objective`, `outcome`, `costUsd`, `numTurns`, `durationMs`, `denialCount`; `null` while non-terminal). Expand an entry with `GET /api/runs/:id` |
| `GET /api/runs/:id` | run detail: state, snapshots, activity projection, `segments`, usage, **summary**, error | activity and `segments` (the turn's ordered bubbles — used to seed a mid-run turn on open, F) derived on read (06 §2); summary per FR-42. "Snapshots" = caps, policy **and the role's instructions** (B5-04) — what the run actually ran under, which agent configs being gitignored (SEC-10) makes unanswerable anywhere else |
