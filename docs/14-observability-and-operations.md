...
| **CLI version bump** (substrate updates 2.1.207) | re-run the S-01 package → sanitize → refresh fixtures → run the `RuntimeAdapter` contract test; **also re-run [S-04](./spikes/S-04/RESULTS.md) and [S-05](./spikes/S-05/RESULTS.md)** — both probe flag behavior no offline suite can see. S-04: the role still reaches the model (a version that parses `--append-system-prompt` and drops it runs every conversation under no role, tests green). S-05: a settings file still cannot widen a turn's tools past `--allowedTools` (its arm D showed that mechanism is live via `permissions.allow`, so a version widening through a key a template writes would end the flag's authority). Only then accept the new version (R-02) |
