...
| N3b-2 ✅ | **Direct conversation with a specialist** (done, PR #69): conversation `mode` + nullable `projectId` (migration 006), a run resolves its session from the specialist's binding (starting it if offline — needs shared-terminal#429), runs serialize per workspace (I-2), frontend chat | `domain/types.ts` (+`workspaceKeyFor`), `store/migrations/006` + `migrate` FK toggle, `orchestrator` (`createSpecialistConversation`, `resolveRunSession`, per-workspace dispatch), `api`, frontend | inherited (start: [#429](https://github.com/gatof81/shared-terminal/issues/429)) | 06, 08 |
