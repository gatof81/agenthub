...
| R-06 | Runaway consumption: loops/huge contexts burn tokens unattended | OPEN | M | M | Per-run caps from day 1: `--max-turns` + wall-clock timeout are the always-on bounds; a dollar budget cap is optional (off by default, ADR-003 amended 2026-07-19 — moot on a subscription); `UsageRecord` per run (Phase 1, adjustment A3) | Caps are per-run; cross-run budgets arrive with Phase 3-4 | 1 |
...
| R-11 | Workspace races: agent run vs human typing in the same project workspace | carried; **agent-vs-agent races designed away by ADR-005** (conversations share the project session; runs serialize per project, I-2) | M | M | Serialize runs per project (Q-03/Q-11 queue); surface "agent working" in the terminal-adjacent UI; document the manual-intervention contract | Human-vs-agent races remain possible by design (shared workspace is a feature) | 1 |
