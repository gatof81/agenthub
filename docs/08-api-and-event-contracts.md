...
| `GET /api/conversations/:id` | detail + messages (paged) | `?before=<messageId>&limit=`. An assistant message whose turn used tools also carries `segments` — the ordered `text`/tool bubbles derived from the run's events (A) — so a reloaded turn renders as the same sequence a live one does; `content` stays the single-blob fallback |
