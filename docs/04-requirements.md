...
| FR-45 | M | A project declares its **workspace**: the substrate `sessionTemplateId` and an optional **`repo`** (`{url, ref?, target?, auth}`), passed to the seam's session config at provisioning. Provisioning reads the workspace from the **project**, never from the agent — an agent is a stateless role, reusable across projects, and carries no repository, template, or session (ADR-006, 18 §2) | ADR-006, owner direction (2026-07-16) |
...
| FR-47 | M | Each project's repo credential is a **fine-grained PAT scoped to that one repository**, held only in the seam's encrypted session config — never in the Hub's database, logs, or run events. One project's credential must not reach another's repository | owner decision (2026-07-16), ADR-006, SEC-11 |
...
| SEC-11 | M | The Hub holds a credential that can **write to the owner's source repositories** — a capability nothing else it holds has (the substrate JWT reaches sessions, the Anthropic token bills, the R2 token reaches snapshots; none touch source). Blast radius is bounded by construction: one fine-grained PAT per repository, so a leak reaches one repo, and revocation is per project. The Hub stores no plaintext — repo auth lives encrypted in the seam's session config | owner decision (2026-07-16), ADR-006, 10 §2 |
